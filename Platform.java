...
public class Platform extends JFrame
{
    ...
    public Platform() {
        ...
        getContentPane().add(new Content(d.width,d.height));

        // setup the frame 	
        setBounds(0,0,d.width,d.height + 23);
        setVisible(true);
    }
...
class Content extends JPanel implements MouseListener, KeyListener, MouseMotionListener{
    ...
    private void makeFormationMovie()
    {
        ...
        for(int i = 1; i < 64 && inFormationMovie; i++) //64 iteration cap for speed.
        {
        ...
        switch (k.getKeyChar())
        {
            ...
            case 'c':
                mandelbrot.modifyBounds(mandelbrot.getRealInv(target.getReal()),
                                        mandelbrot.getImaginaryInv(target.getImaginary()),
                                        1);
                break;
                //Change Zoom factor
            case 'f':
                ...
                break;
                //Change number of iterations and pallete
            case 'i':
            ...
            case 'l':
                JOptionPane.showMessageDialog(this, 
                        ...
                        new ComplexNumber(Fractal.getReal(WIDTH/2),
                            Fractal.getImaginary(HEIGHT/2)),
                        "Current Centered Point", 1, null);
                break;
                //Create a movie!
            case 'm':
                movie = true;
                target = ComplexNumber.parseComplexNumber(
                        JOptionPane.showInputDialog(
                            "What is your desired target?"));
                if (target == null)
                    return;
            ...
            case 'p':
                ...
                if (pnum == null || 
                        pnum.equals("") ||
                        Character.isLetter(pnum.charAt(0)) ||
                        pnum.length() >= 2 || pnum.length() < 1 ||
                        Integer.parseInt(pnum) > 8)
                    return;
                if (pnum.equals("2"))
                {
                    String seed = JOptionPane.showInputDialog("Any specific seed?");
    ...
    public void mouseReleased(MouseEvent e)
    {
        if (isBoxMode)
        {
            ...
            mandelbrot.modifyBounds(firstpoint.getX() + ((lastpoint.getX() - firstpoint.getX())/2),
                                    ...
                                    factor);
            //Re-calculate the onScreen values
            onScreen = (new ComplexNumber(Fractal.getReal(firstpoint.getX() +
                                            ((lastpoint.getX() - firstpoint.getX())/2)),
                                          Fractal.getImaginary(
                                              firstpoint.getY() + 
    ...
    public void mouseDragged(MouseEvent e)
    {
        if (isBoxMode)
        {
            ...
            {
                //nuke the old lines.
                g2.drawLine(firstpoint.getX(), firstpoint.getY(),
                            previousdrag.getX(), firstpoint.getY());
                g2.drawLine(previousdrag.getX(), firstpoint.getY(),
                            previousdrag.getX(), previousdrag.getY());
                g2.drawLine(previousdrag.getX(), previousdrag.getY(),
                            firstpoint.getX(), previousdrag.getY());
                g2.drawLine(firstpoint.getX(), previousdrag.getY(),
    ...
    public void screenCapture()	{ 
        ...
        File file;
        if (!movie)
            file = new File(JOptionPane.showInputDialog("What do you want the file to be named",
                            (new ComplexNumber(Fractal.getReal(WIDTH/2), 
                                               Fractal.getImaginary(HEIGHT/2) ))) + ".png");
        else
